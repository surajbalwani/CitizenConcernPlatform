...
export class ApiService {
  ...
  register(request: RegisterRequest): Observable<{ message: string; userId: string }> {
    return this.http.post<{ message: string; userId: string }>(`${this.apiUrl}/auth/register`, request)
  ...
  getConcerns(params?: QueryParams): Observable<PaginatedResponse<Concern>> {
    ...
    return this.http.get<Concern[]>(`${this.apiUrl}/concerns`, { 
      params: httpParams,
      observe: 'response'
    }).pipe(
      map(response => {
  ...
  getConcern(id: number): Observable<Concern> {
    return this.http.get<Concern>(`${this.apiUrl}/concerns/${id}`)
      .pipe(catchError(this.handleError.bind(this)));
  }

  createConcern(request: CreateConcernRequest): Observable<Concern> {
    return this.http.post<Concern>(`${this.apiUrl}/concerns`, request)
      .pipe(catchError(this.handleError.bind(this)));
  }

  updateConcernStatus(id: number, request: UpdateStatusRequest): Observable<void> {
    return this.http.put<void>(`${this.apiUrl}/concerns/${id}/status`, request, {
      headers: this.getAuthHeaders()
    }).pipe(catchError(this.handleError.bind(this)));
  }

  voteConcern(id: number, request: VoteRequest): Observable<{ upVotes: number; downVotes: number; priority: number }> {
    return this.http.post<{ upVotes: number; downVotes: number; priority: number }>(
  ...
  addComment(id: number, request: AddCommentRequest): Observable<any> {
    return this.http.post(`${this.apiUrl}/concerns/${id}/comments`, request)
      .pipe(catchError(this.handleError.bind(this)));
  }

  getConcernAnalytics(): Observable<ConcernAnalytics> {
    return this.http.get<ConcernAnalytics>(`${this.apiUrl}/concerns/analytics`, {
      headers: this.getAuthHeaders()
    }).pipe(catchError(this.handleError.bind(this)));
  }
