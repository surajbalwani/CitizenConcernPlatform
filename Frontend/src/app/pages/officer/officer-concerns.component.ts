...
import { Concern, PaginatedResponse, ConcernStatus, UserResponse, UserRole, UpdateStatusRequest } from '../../core/models/api.models';
...
@Component({
              ...
              <mat-card-content>
                <div class="officer-details">
                  ...
                  <div class="officer-text">
                    <h3>{{ currentOfficer()?.firstName }} {{ currentOfficer()?.lastName }}</h3>
                    <p class="department">{{ currentOfficer()?.department || 'General' }}</p>
        ...
        <mat-card-content>
          <div class="filters-container">
            ...
            <mat-form-field>
              ...
              <mat-input [(ngModel)]="searchQuery" (input)="applyFilters()" placeholder="Search by title, description..."></mat-input>
              <mat-icon matSuffix>search</mat-icon>
            </mat-form-field>

            <button mat-raised-button color="primary" (click)="refreshConcerns()">
      ...
      <mat-card class="concerns-card">
        <mat-card-header>
          <mat-card-title>
            ...
            My Assigned Concerns
            @if (concernStats().assigned > 0) {
              <mat-chip-set>
                <mat-chip [color]="'accent'">{{ concernStats().assigned }} Active</mat-chip>
              </mat-chip-set>
            }
          </mat-card-title>
        </mat-card-header>
        ...
        <mat-card-content>
          @if (isLoading()) {
            ...
            <div class="error-container">
              ...
              <p>{{ errorMessage() }}</p>
              <button mat-raised-button color="primary" (click)="loadOfficerConcerns()">
                <mat-icon>refresh</mat-icon>
                Retry
              </button>
            </div>
              ...
              <div class="concerns-table-container">
                <table mat-table [dataSource]="concerns().data" class="concerns-table">
                  ...
                  <ng-container matColumnDef="priority">
                    ...
                    <td mat-cell *matCellDef="let concern">
                      <span [class]="'priority-' + concern.priority.toLowerCase()">
                        {{ concern.priority }}
                      </span>
                    </td>
                  </ng-container>
  ...
  styles: [`
    ...
    .stat-card.assigned .stat-content mat-icon { color: #2196f3; }
    .stat-card.in-progress .stat-content mat-icon { color: #ff9800; }
    .stat-card.resolved .stat-content mat-icon { color: #4caf50; }
    .stat-card.urgent .stat-content mat-icon { color: #f44336; }
    ...
    .loading-container, .error-container, .no-concerns {
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
    ...
    .error-container mat-icon, .no-concerns mat-icon {
      font-size: 48px;
      width: 48px;
      height: 48px;
      margin-bottom: 16px;
...
export class OfficerConcernsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'title', 'category', 'status', 'priority', 'date', 'actions'];
    ...
    this.apiService.getConcerns(params).subscribe({
      next: (response) => {
        ...
        if (currentUser.role === UserRole.Officer) {
          ...
          officerConcerns = response.data.filter(concern => 
  ...
  updateStats(concerns: Concern[]) {
    const stats = {
      assigned: concerns.filter(c => c.status === ConcernStatus.Acknowledged).length,
  ...
  viewConcernDetails(concern: Concern) {
    ...
    this.snackBar.open(`Viewing concern #${concern.id}: ${concern.title}`, 'Close', {
      duration: 3000
    });
  }
  ...
  updateConcernStatus(concern: Concern) {
    ...
    this.apiService.updateConcernStatus(concern.id, updateRequest).subscribe({
      next: () => {
        this.snackBar.open(`Concern #${concern.id} status updated to ${nextStatus}`, 'Close', {
          duration: 3000
        });
