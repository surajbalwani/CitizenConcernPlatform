      ...
      <mat-card class="location-card">
        <mat-card-content>
          ...
          <div class="location-status success">
            ...
            <span
              >Location found! Showing nearby concerns within
              {{ radiusKm }}km</span
            >
          </div>
          } @else {
          ...
          <div class="filters-row">
            <mat-form-field>
              ...
              <mat-select
                ...
                (selectionChange)="loadNearbyConcerns()"
              >
      ...
      <div class="concerns-section">
        ...
        <mat-card class="empty-state">
          <mat-card-content>
            ...
            <p>
              There are no reported concerns within {{ radiusKm }}km of your
              location.
            </p>
            <button
              mat-raised-button
        ...
        <div class="results-header">
          <h3>
            {{ concerns().totalCount }} concerns found within {{ radiusKm }}km
          </h3>
        </div>
        ...
        <div class="concerns-grid">
          ...
          <mat-card
            class="concern-card"
            [class.own-concern]="isOwnConcern(concern)"
          >
            <mat-card-header>
            ...
            <mat-card-content>
              ...
              <div class="voting-section">
                <button
                  ...
                  matTooltip="{{
                    isOwnConcern(concern)
                      ? 'Cannot vote on your own concern'
                      : 'Upvote this concern'
                  }}"
                >
...
export class NearbyConcernsComponent implements OnInit {
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            this.errorMessage.set(
              'Location access denied. Please enable location services to see nearby concerns.'
            );
            break;
  ...
  private calculateDistanceFromLocation(
    lat1: number,
    lon1: number,
    lat2: number,
    lon2: number
  ...
  upvoteConcern(concern: Concern) {
    ...
    this.apiService.voteConcern(concern.id, voteRequest).subscribe({
      next: (response) => {
        ...
        const concernIndex = this.concerns().data.findIndex(
          (c) => c.id === concern.id
        );
        if (concernIndex !== -1) {
  ...
  getDirections(concern: Concern) {
    if (concern.location && this.currentLocation()) {
      const url = `https://www.google.com/maps/dir/${
        this.currentLocation()!.latitude
      },${this.currentLocation()!.longitude}/${concern.location.latitude},${
        concern.location.longitude
      }`;
  ...
  shareConcern(concern: Concern) {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(
        `${concern.title}: ${window.location.href}`
      );
