...
@Component({
  ...
  template: `
    <div class="mobile-container" [class.mobile-layout]="layoutService.isMobile()">
      ...
      <mat-stepper #stepper orientation="vertical" [linear]="true">
        <mat-step [stepControl]="basicInfoForm" label="Basic Information">
          <form [formGroup]="basicInfoForm">
            ...
            <mat-form-field class="full-width">
              ...
              <button mat-icon-button matSuffix (click)="startVoiceInput()" [disabled]="isListening()">
        ...
        <mat-step [stepControl]="locationForm" label="Location">
          <form [formGroup]="locationForm">
            <mat-form-field class="full-width">
              ...
              <button mat-icon-button matSuffix (click)="getCurrentLocation()" [disabled]="gettingLocation()">
                <mat-icon>{{gettingLocation() ? 'gps_fixed' : 'location_on'}}</mat-icon>
              </button>
            </mat-form-field>
            ...
            <div class="form-row">
              ...
              <mat-form-field class="half-width">
                ...
                <input matInput formControlName="ward" placeholder="Local ward or area">
              </mat-form-field>
            </div>
        ...
        <mat-step label="Evidence">
          <div class="media-section">
            <button mat-raised-button (click)="capturePhoto()" class="full-width photo-button">
              ...
              <div class="image-preview">
                @for (image of capturedImages(); track image.id) {
                  <div class="image-container">
                    ...
                    <button mat-mini-fab color="warn" class="remove-button" (click)="removeImage(image.id)">
            ...
            <mat-form-field class="full-width">
              ...
              <textarea matInput [(ngModel)]="additionalNotes" rows="3" placeholder="Any additional information"></textarea>
            </mat-form-field>
          </div>
        ...
        <mat-step label="Review & Submit">
          <div class="review-section">
            <mat-card>
              ...
              <mat-card-content>
                ...
                <p><strong>Region:</strong> {{locationForm.get('region')?.value}}</p>
                <p><strong>Ward:</strong> {{locationForm.get('ward')?.value}}</p>
                <p><strong>Anonymous:</strong> {{locationForm.get('isAnonymous')?.value ? 'Yes' : 'No'}}</p>
                <p><strong>Photos:</strong> {{capturedImages().length}} attached</p>
                @if (additionalNotes) {
          ...
          <div class="step-actions">
            <button mat-button matStepperPrevious>Back</button>
            <button mat-raised-button color="primary" (click)="submitConcern()" [disabled]="isSubmitting()">
              @if (isSubmitting()) {
...
export class SubmitConcernComponent implements OnInit {
  ...
  startVoiceInput() {
    if (this.isListening()) {
      ...
      setTimeout(() => {
        ...
        const currentDescription = this.basicInfoForm.get('description')?.value || '';
        this.basicInfoForm.patchValue({
          description: currentDescription + ' [Voice input would be added here]'
        });
      }, 3000);
    }
  }
  ...
  removeImage(imageId: string) {
    this.capturedImages.update(images => images.filter(img => img.id !== imageId));
  }
  ...
  submitConcern() {
    if (this.basicInfoForm.valid && this.locationForm.valid) {
      ...
      const concernRequest: CreateConcernRequest = {
        ...
        description: basicInfo.description + (this.additionalNotes ? '\n\nAdditional Notes: ' + this.additionalNotes : ''),
        address: locationInfo.address,
        region: locationInfo.region,
        ward: locationInfo.ward,
        citizenId: currentUser.id,
        citizenName: isAnonymous ? undefined : `${currentUser.firstName} ${currentUser.lastName}`,
      ...
      this.apiService.createConcern(concernRequest).subscribe({
        ...
        error: (error) => {
          this.isSubmitting.set(false);
          this.errorMessage.set(error.message || 'Failed to submit concern. Please try again.');
          console.error('Failed to submit concern:', error);
        }
      });
    } else {
      this.errorMessage.set('Please fill in all required fields before submitting.');
    }
  }
}